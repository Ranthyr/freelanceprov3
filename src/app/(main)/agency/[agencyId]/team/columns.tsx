...
export const columns: ColumnDef<UsersWithAgencySubAccountPermissionsSidebarOptions>[] =
  [
    ...
    },
    {
      accessorKey: 'name',
      header: 'Name',
      cell: ({ row }: { row: any }) => { // Explicitly type the row
...

    {
      ...
      cell: ({ row }: { row: any }) => { // Explicitly type the row
        ...
        const ownedAccounts = row.original?.Permissions.filter(
          (per: any) => per.access // Explicitly type per
        )
          ...
          <div className="flex flex-col items-start">
            <div className="flex flex-col gap-2">
              {ownedAccounts?.length ? (
                ownedAccounts.map((account: any) => ( // Explicitly type account
                  <Badge
                    key={account.id}
    ...
    },
    {
      id: 'actions',
      cell: ({ row }: { row: any }) => { // Explicitly type the row
        const rowData = row.original as UsersWithAgencySubAccountPermissionsSidebarOptions
