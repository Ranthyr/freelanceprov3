...
const SubaccountPageId = async ({ params, searchParams }: Props) => {
  ...
  if (subaccountDetails.connectAccountId) {
    ...
    totalClosedSessions = checkoutSessions.data.filter(
      (session: Stripe.Checkout.Session) => session.status === 'complete'
    )

    totalPendingSessions = checkoutSessions.data.filter(
      (session: Stripe.Checkout.Session) => session.status === 'open' || session.status === 'expired'
    )

    net = +totalClosedSessions
      .reduce((total, session: Stripe.Checkout.Session) => total + (session.amount_total || 0), 0)
      .toFixed(2)
              ...
              <CardHeader>
                ...
                <Table>
                  ...
                  <TableBody className="font-medium truncate">
                    {totalClosedSessions
                      ? totalClosedSessions.map((session: Stripe.Checkout.Session) => (
